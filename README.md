...
No computer is perfectly safe, especially when connected to the Internet. This guide is meant to give activists a few tips to increase the security, using the example of a Mac OS device (although should apply to Windows). The guide is not meant to be complete: cybersecurity and convenience is a tradeoff, and this guide will try to find a good balance to keep you moderately secure, but still able to use the everyday computing tools you need to conduct activism work. I assume your threat model is medium (Recommended) to intermediate (Marked *“Optional”*)
...
As per the guide, I recommend installing the NoScript plug-in. This may be a bit of an annoyance but it is well worth it. Note that if you use services like online banking, you can temporarily or permanently enable JavaScript or cookies.
...
Install [Tor Browser](https://www.torproject.org/) on your machine as a separate Internet browser. Based on a fork of the Firefox browser, it uses The Onion Router to keep your browsing activity as anonymous and untrackable as possible (your real IP address and hence location will be scrambled), although there are things we should take precaution about later.

You can consider using the Tor Browser for anything you want to keep really private, such as conducting research, or logging into sensitive web email / social media accounts for activism activities. **Important:** If you use the Tor browser to log into, say, your activist Twitter account, and also log into the same account using a normal non-Tor browser, the social media service may be able to identify and *unscramble* you based on their logged IP address information. You should therefore decide whether you will settle on using VPN+Container approach to manage your activist account, or always do it through Tor Browser.

Another risk of Tor Browser (that is less of an issue now) is that the authorities can detect who is using it (via analyzing the exit nodes that connect you to the Tor network). In countries like the US when there is a huge number of daily Tor users, it is less of a risk, but for countries like Hong Kong, where the usage can be in the thousands only, it is advisable to use Tor only after connecting to a VPN in another country.
...
While the most secure way to email is using local email clients with strong encryption keys, services like Proton Mail can be a really easy way to securely communicate information between journalists, activists, political figures, without having to invest a lot of time and energy on setting up encryption keys locally.
